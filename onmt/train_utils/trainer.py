...
class XETrainer(BaseTrainer):
    ...
    def eval(self, data):
        ...
        # normalize weights
        w_reconstr, w_adv, w_classif = opt.w_reconstr / weight_sum, opt.w_adv / weight_sum, opt.w_classif / weight_sum
        ...
        for i in range(iteration, nSamples):
            ...
            try:
                def train_part(total_loss_f):
                    ...
                    if opt.save_grad is not None and i <= 1:  # only show for first two batches in epoch
    ...
    def run(self, save_file=None):
        ...
        for epoch in range(opt.start_epoch, opt.start_epoch + opt.epochs):
            ...
            p = (epoch - opt.start_epoch + 1) / opt.reconstr_headstart if opt.reconstr_headstart != 0 else 0
