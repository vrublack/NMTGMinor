...
class CheckpointFunction(Function):
    ...
    def backward(ctx, *grads):
        with torch.enable_grad():
            ...
            filtered_inputs = [i for i in inputs if requires_grad(i)]
            ...
            return output
            #~ grads = []
            #~ for i, input_ in enumerate(filtered_inputs):
                #~ print(input_.grad)
                #~ grads.append(input_.grad.data)

            # Append None for input grads which don't require grad. The first input
            # is a run_function whose grad is None
            #~ grads_it = iter(grads)
            #~ return (None,) + tuple(
                #~ i.grad.data if requires_grad(i) and i.grad is not None else None for i in inputs)
            #~ return (None,) + tuple(
